import { isConfigured, patchSale } from './_lib/supabase.js';
...
export default async function handler(req,res){
  ...
  try{
    ...
    if(!['ok','not_ok'].includes(status)) return json(res,400,{error:'Status de auditoria inválido.'});
    const sale=await patchSale(saleId,{audit_status:status,audited_by:auditedBy,audited_at:new Date().toISOString()});
